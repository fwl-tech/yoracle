  ...
  return (
    <div className="min-h-screen bg-gray-950">
      <nav className="border-b border-gray-800 px-6 py-4 flex items-center gap-4">
        <Link href="/digest" className="text-gray-400 hover:text-white text-sm transition">&larr; Digest</Link>
          ...
          {dashboards.map(d => (
            <Link key={d.id} href={`/dashboards/${d.id}`}>
              <div className="bg-gray-900 border border-gray-800 hover:border-gray-600 rounded-2xl p-6 cursor-pointer transition space-y-2">
                ...
                <p className="text-xs text-brand-400">{d.kpi_ids.length} KPIs &rarr;</p>
              </div>
            </Link>
          ))}
        </div>
